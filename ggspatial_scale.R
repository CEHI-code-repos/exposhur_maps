...
GeomScaleBar <- ggplot2::ggproto(
  ...
  draw_panel = function(
    ...
    stopifnot(
      ...
      length(unit_category) == 1,
      unit_category %in% c("metric", "imperial"),
      is.numeric(width_hint),
      length(width_hint) == 1,
      is.atomic(bar_cols),
      ...
      grid::is.unit(pad_y),
      length(pad_y) == 1,
      grid::is.unit(text_pad),
      length(text_pad) == 1,
      length(text_col) == 1,
      is.numeric(tick_height),
      length(tick_height) == 1
    )
    ...
    if (inherits(coordinates, "CoordSf")) {
      ...
      stop(
        "Don't know how to create scalebar using ",
        paste(class(coordinates), collapse = "/")
      )
    }
...
scalebar_grobs <- function(
  ...
  text_origin_x <- unit(adj_x, "npc") +
    (0.5 - adj_x) * 2 * (pad_x + text_pad + width)
  text_origin_y <- unit(adj_y, "npc") +
    (0.5 - adj_y) * 2 * (pad_y + 0.5 * height)

  if (style == "bar") {
    bar_grob <- grid::rectGrob(
      x = origin_x +
        unit((seq_len(params$majordivs) - 1) * params$majordivnpc, "npc"),
      y = origin_y,
    ...
    bar_grob <- grid::gList(
      grid::segmentsGrob(
        x0 = origin_x +
          unit((seq_len(params$majordivs + 1) - 1) * params$majordivnpc, "npc"),
        y0 = origin_y,
        x1 = origin_x +
          unit((seq_len(params$majordivs + 1) - 1) * params$majordivnpc, "npc"),
        y1 = origin_y +
          grid::unit.c(
            height,
            rep(height * tick_height, params$majordivs - 1),
            height
          ),
        gp = grid::gpar(
          lwd = line_width,
...
scalebar_params <- function(
  ...
  if (!is.null(sf_crs) && is.null(plotunit)) {
    point_coords <- expand.grid(
      ...
      y = c(
        sf_bbox["ymin"],
        mean(c(sf_bbox["ymin"], sf_bbox["ymax"])),
        sf_bbox["ymax"]
      )
    )
    ...
    percentdiff <- (max(widthbottom, widthmiddle, widthtop) -
      min(widthbottom, widthmiddle, widthtop)) /
      min(widthbottom, widthmiddle, widthtop)
